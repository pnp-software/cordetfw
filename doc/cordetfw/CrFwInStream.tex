...
The InStreams are responsible for checking the sequence counter attributes of incoming packets received by an application. The InStream assumes that the packets it receives have increasing sequence counters.
...
Also as part of the processing of the \texttt{PacketAvailable} command, the \textit{Packet Collect Procedure} checks the sequence counter attribute of incoming packets which have the host application as their destination. The procedure assumes that the sequence counter increases by one for each packet it receives. If the procedure finds that the sequence counter has not incremented by one, it reports the sequence counter error. 
