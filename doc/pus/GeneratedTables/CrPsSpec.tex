...
Description & Command to enable time-based schedule execution function \\\hline
Parameters & None \\\hline
Discriminant & None \\\hline
Ready Check & Default implementation (command is always ready) \\\hline
...
Description & Command to disable time-based schedule execution function \\\hline
Parameters & None \\\hline
Discriminant & None \\\hline
Ready Check & Default implementation (command is always ready) \\\hline
Start Action & Default implementation (set action outcome to 'success') \\\hline
...
Discriminant & None \\\hline
Ready Check & Default implementation (command is always ready) \\\hline
Start Action & Run the procedure Start Action of (11,5) Command of figure \ref{fig:Cmd11s5Start}. This procedure iterates over the list of activity identifiers in the command and rejects those which do not exist.  \\\hline
Progress Action & For each activity identifier which has been accepted by the Start Action, the following is done:\newline \newline (a) The slot in the TBS where the activity is stored is cleared (its release time is set to zero)\newline (b) The number of TBAs in the TBS (nOfTba) is decremented by 1\newline (c) The number of TBAs in the sub-schedule (nOfTbaInSubSched) to which the deleted activity belongs is decremented by 1\newline (d) If the deleted activity was the last one in the sub-schedule, then the number of non-empty sub-schedules (nOfSubSched) is decremented by one\newline (e) The number of TBAs in the groupd (nOfTbaInGroup)  to which the deleted activity belongs is decremented by 1\newline \newline After all identifiers have been processed, the action outcome is set to: 'completed'. \\\hline
Termination Action & Default implementation (set action outcome to 'success') \\\hline
Abort Action & Default implementation (set action outcome to 'success') \\\hline
\end{pnptable}}
