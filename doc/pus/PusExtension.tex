...
Three procedures are defined to handle the creation, configuration and loading of the service 1 reports: (a) the Packet Rerouting Failure Procedure of figure \ref{fig:PcktReroutingFail} handles the (1,10) report; (b) the Command Verification Failure Procedure of figure \ref{fig:CmdVerFail} handles the failure reports; and (c) the Command Verification Success Procedure of figure \ref{fig:CmdVerSucc} handles the success reports. The close-out of the adaptation points listed above consists in running one of these procedures. 
...
The transfer of the failure-related data item from the component where the command verification check is performed to the procedures of figures \ref{fig:PcktReroutingFail} to \ref{fig:CmdVerSucc} is done via the data pool: the component performing the verification check determines the value of the failure-related data item and stores it in the data pool and the command verification procedure takes it from the data pool and puts it in the service 1 report.

The progress reports (1,5) and (1,6) also carry the identifier of the step being verified. A similar mechanism is used for the transfer of the step information: the component performing the verification check stores the identifier of the step being verified in the data pool and the command verification procedure takes it from the data pool and puts it in the service 1 report.
...
The table in this section lists the adaptation points for the request verification service. The following remarks apply: TBD.
