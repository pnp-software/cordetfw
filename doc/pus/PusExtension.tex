...
There are some terminological differences between PUS and CORDET. For clarity, table \ref{tab:termPusCr} lists PUS-specific and gives the corresponding term or concept in the CORDET world. 

\begin{longtable}{|l|>{\raggedright\arraybackslash}p{6cm}|}
...
\hline\hline
\endhead
Instruction & In the PUS, a request (command) contains one or more instructions. Instructions do not exist in CORDET and are therefore implicit to a command. \\
\hline
...
\hline
\rowcolor{light-gray}
\textbf{Field Name} & \textbf{Description} & \textbf{Remarks} \\
\hline\hline
\endfirsthead
\rowcolor{light-gray}
\textbf{Field Name} & \textbf{Description} & \textbf{Remarks} \\
...
\hline
\texttt{period} & Collection period in units of HK\_COLLECT\_PER & A value of zero indicates a "one-shot" report \\
\hline
...
In the PUS Service 3 commands and reports appear twice once for housekeeping reports and once for diagnostic reports. The CORDET Framework does not distinguish between housekeeping and diagnostic reports and therefore each CORDET report/command component implements two PUS reports/commands:

\begin{fw_itemize}
\item Component HkCreate implements commands (3,1) and (3,2) to create a report structure
\item Component HkDelete implements commands (3,3) and (3,4) to delete a report structure
\item TBD
\end{fw_itemize}

These components are defined by the way they close the adaptation points of the OutComponent and InCommand. This is defined formally in tables \ref{tab:AP-S3-1} to TBD.
...
\begin{cr_ap}{S3-1}{Adaptation Points for (3,1) Command}
\end{cr_ap}
