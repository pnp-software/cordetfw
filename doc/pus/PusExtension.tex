...
The verification reports are generated at various points in the processing life-cycle of an incoming command. The CORDET Framework defines adaptation points where the success or failure of a processing step of an incoming command is handled. The framework extension closes these adaptation points with behaviour which generates the verification reports. 



\begin{longtable}{|c|>{\raggedright\arraybackslash}p{6.0cm}|>{\raggedright\arraybackslash}p{5.0cm}}
\caption{Request Verification Service Components} \label{tab:serv1Comp}\\
\hline
\rowcolor{light-gray}
\textbf{Type} & \textbf{PUS Report} & \textbf{Framework Component} \\
\hline\hline
\endfirsthead
\rowcolor{light-gray}
\textbf{Type} & \textbf{PUS Report} & \textbf{Framework Component} \\
\hline\hline
\endhead
TM(1,1) & Successful Acceptance Verification Report & VerAcceptSuccess \\
\hline
\end{longtable}
