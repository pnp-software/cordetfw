...
\begin{itemize}
\item An entry is made in the Checking Transition List (see section \ref{sec:ChkTransList})
...
The overall enable status of a parameter monitor can be controlled at global level (the Monitoring Function Procedure is started/stopped which enables/disables the entire monitoring function) or at local level (a specific Parameter Monitor State Machine and Procedure are started/stopped).
...
The Check Transition List (CTL) is a data structure consisting where the monitoring violations are accumulated. It consists of MON\_N\_CLST entries where each entry has the attributes listed in table \ref{tab:chkTransAttr}.

Entries are added to the CTL by the Monitoring Function Procedure (see figure \ref{fig:MonFncPr}) when it wishes to report a monitoring violation. The CTL is cleared by the Update Action of the TBD report. This report is generated either autonomously when the list becomes full or in response to command TBD.

If the list is full when the "add entry to CTL" operation is called, event EVT\_CLST\_FULL is generated.
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.55,keepaspectratio=true]{CrPsMonFncPr.png}
 \caption{Parameter Monitoring Procedure}
 \label{fig:MonFncPr}
\end{figure}
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.55,keepaspectratio=true]{CrPsMonLimCheckPr.png}
 \caption{Limiting Check Monitor Procedure}
 \label{fig:MonLimCheckPr}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[scale=0.55,keepaspectratio=true]{MonExpValPr.png}
 \caption{Expected Value Monitor Procedure}
 \label{fig:MonExpValPr}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[scale=0.55,keepaspectratio=true]{MonDeltaValPr.png}
 \caption{Delta Value Monitor Procedure}
 \label{fig:MonDeltaValPr}
\end{figure}
