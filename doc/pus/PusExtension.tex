...
\item Delta Check Monitor Procedure: verifies whether the difference between the current and previous value of the monitored parameter is within a pre-defined interval. See figure \ref{fig:MonDeltaPr}.
\end{itemize}
...
Entries are added to the CTL by the Monitoring Function Procedure (see figure \ref{fig:MonFncPr}) when it wishes to report a monitoring violation. If the list is full when the "add entry to CTL" operation is called, event EVT\_CLST\_FULL is generated.

The CTL is processed by the MonCheckTransRep report (12,12). Applications should load this report as part of their initialization. The report is always enabled and remains permanently pending. By default it is not ready but, every time it is executed, it checks whether one of the following conditions holds:

\begin{itemize}
\item The CTL contains at least MON\_CTL\_NLIM entries
\item The CTL has not been flushed for longer than MON\_CTL\_DEL monitoring cycles of duration MON\_PER
\end{itemize}
 
If either condition is satisfied, the MonCheckTransRep report is declared to be ready and is therefore sent to its destination. Its update action collects the content of the report from the CTL and then clears the CTL.
...
%-------------------------------------------------------------------------------------
\pnpfigure[scale=0.55]{Monitoring Function Procedure}{fig:MonFncPr}{CrPsMonFncPr.png}
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.55,keepaspectratio=true]{CrPsCmd12PMonIdStart.png}
 \caption{Start Action of Multi-Parameter Monitor Commands}
 \label{fig:Cmd12PMonIdStart}
\end{figure}
