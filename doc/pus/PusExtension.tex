...
Entries are added to the CTL by the Monitoring Function Procedure (see figure \ref{fig:MonFncPr}) when it wishes to report a monitoring violation. For this purpose, the framework provides an operation to add an entry to the CTL. When the first entry is added to the CTL, variable \texttt{ctlTimeFirstEntry} is updated with the current time.
...
If either condition is satisfied, the procedure generates the MonCheckTransRep report (12,12) to send the current content of the CTL to the service 12 user. The procedure then clears the CTL and sets \texttt{ctlTimeFirstEntry} to a value very far in the future.
...
\pnpfigure[scale=0.5]{Start Action of Multi-Parameter Monitor Commands}{fig:Cmd12PMonIdStart}{CrPsCmd12PMonIdStart.png}
