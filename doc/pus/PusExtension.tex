...
Different LPT Buffers may have different values of \texttt{lptDest} or \texttt{partSize}. This allows the application designers to, for instance, allocate some LPT Buffers to large transfers to/from the ground and other LPT Buffers to large transfers to/from other destinations. Or, it allows them to use different LPT Buffers for different large packet size. Also, \texttt{lptDest} and \texttt{partSize} are parameters (see section TBD). Their values can therefore be adjusted by the service 13 user. It is the user responsibility to avoid changing the value of \texttt{lptDest} or \texttt{partSize} for a given LPT Buffer while a transfer to or from that buffer is under way.
