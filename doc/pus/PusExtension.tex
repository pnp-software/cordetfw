...
\newpage
\begin{longtable}{|>{\raggedright\arraybackslash}p{4cm}|>{\raggedright\arraybackslash}p{7.7cm}|c|}
...
Several service 3 commands operate on a set of RDL entries (e.g. command (3,3) requestes that a list of SIDs be deleted from the RDL). In such cases, it is useful to "mark" an entry in the RDL. For this purpose, flag \texttt{isMarked} has been introduced in table \ref{tab:repDefDataStruct}.
...
\pnpcsvtable[filter equal={\Domain}{Hk}]{|p{3cm}|c|>{\raggedright\arraybackslash}p{8.5cm}|}{Observables and Parameters for Housekeeping Service}{tab:Obs-S3}{Name & Kind & Description}{./GeneratedTables/Datapool.csv}{\texttt{\Name} & \Kind & \ShortDesc}
...
\newpage
\printEvtEnableCmdSpec{|c|p{10cm}|}
\printEvtDisableCmdSpec{|c|p{10cm}|}
\newpage
\printEvtRepDisabledCmdSpec{|c|p{10cm}|}
\printEvtRepDisabledRepSpec{|c|p{10cm}|}
...
\begin{fw_itemize}
\item The checks on the legality of the time-scheduled command are done in the Start Action of the (11,4) command (see table \ref{tab:ScdInsTba} and in particular figure \ref{fig:Cmd11s4Start})
\item The insertion of the TBA in the TBS is done by the Progress Action of the (11,4) command (see table \ref{tab:ScdInsTba})
...
Within the TBS, up to SCD\_N\_SUB\_TBS \textit{sub-schedules} may be defined. Each sub-schedule is identified by an integer in the range 1 to SCD\_N\_SUB\_TBS. The sub-schedule 1 is the default sub-schedule. Thus, an application which does not need sub-schedules should set SCD\_N\_SUB\_TBS to 1. To each TBA, the attribute \texttt{subSchedId} is associated which identifies the sub-schedule to which the TBA belongs.

In the current version of the PUS Extension of the CORDET Framework, only the default sub-schedule is supported and SCD\_N\_SUB\_TBS must be kept equal to 1.


\subsection{Groups}
The TBAs in the TBS may be assigned to groups. Up to SCD\_N\_GROUP groups may be defined. Each group is identified by an integer in the range 1 to SCD\_N\_GROUP. The group 1 is the default group. Thus, an application which does not need groups should set SCD\_N\_GROUPS to 1. To each TBA, the attribute \texttt{groupId} is associated which identifies the group to which the TBA belongs.

In the current version of the PUS Extension of the CORDET Framework, only the default sub-schedule is supported and SCD\_N\_GROUP must be kept equal to 1.
...
\hline
\texttt{nextTba} & The identifier within the TBS of the time-based activity with the next release time or zero if this is the last (fartherst in the future) entry in the TBS & Integer in range 1..SCD\_N\_TBA \\
\hline
...
Tables \ref{tab:TBD} to \ref{tab:TBD} formally specify the service 11 commands and reports by specifying how the actions, checks and attributes of generic out-going commands and reports are specialized for this service (see section \ref{sec:defPusRepCmd}). 
...
\pnpcsvtable[filter equal={\Domain}{Mon}]{|p{3cm}|>{\raggedright\arraybackslash}p{9.5cm}|}{Constants for Time-Based Service}{tab:Const-S11}{Name & Description}{./GeneratedTables/Constants.csv}{\texttt{\Name} & \Desc}
...
\pnpcsvtable[filter equal={\Domain}{Mon}]{|p{3cm}|c|>{\raggedright\arraybackslash}p{8.5cm}|}{Observables and Parameters for Time-Based Scheduling Service}{tab:Obs-S11}{Name & Kind & Description}{./GeneratedTables/Datapool.csv}{\texttt{\Name} & \Kind & \ShortDesc}
...
\pnpcsvtable[filter equal={\Domain}{Mon}]{|p{3cm}|c|>{\raggedright\arraybackslash}p{8.5cm}|}{Observables and Parameters for On-Board Monitoring Service}{tab:Obs-S12}{Name & Kind & Description}{./GeneratedTables/Datapool.csv}{\texttt{\Name} & \Kind & \ShortDesc}
...
\begin{crRep}{S13-1}{Specification of LptDownFirstRep Component}
\end{crRep}

\newpage
\begin{crRep}{S13-2}{Specification of LptDownInterRep Component}
\end{crRep}

\begin{crRep}{S13-3}{Specification of LptDownLastRep Component}
\end{crRep}

\newpage
\begin{crCmd}{S13-9}{Specification of LptUpFirstCmd Component}
\end{crCmd}

\begin{crCmd}{S13-10}{Specification of LptUpInterCmd Component}
\end{crCmd}

\newpage
\begin{crCmd}{S13-11}{Specification of LptUpLastCmd Component}
\end{crCmd}

\begin{crRep}{S13-16}{Specification of LptUpAbortRep Component}
\end{crRep}

\newpage
\begin{crCmd}{S13-129}{Specification of LptStartDownCmd Component}
\end{crCmd}

\begin{crCmd}{S13-130}{Specification of LptAbortDownCmd Component}
\end{crCmd}
...
\pnpcsvtable[filter equal={\Domain}{Lpt}]{|p{2.5cm}|c|>{\raggedright\arraybackslash}p{9cm}|}{Observables and Parameters for LPT Service}{tab:Obs-S13}{Name & Kind & Description}{./GeneratedTables/Datapool.csv}{\texttt{\Name} & \Kind & \ShortDesc}
...
\newpage
\begin{crCmd}{S17-1}{Specification of AreYouAliveCmd Component}
\end{crCmd}

\begin{crRep}{S17-2}{Specification of AreYouAliveRep Component}
\end{crRep}

\newpage
\begin{crCmd}{S17-3}{Specification of OnBoardConnectCmd Component}
\end{crCmd}

\begin{crRep}{S17-4}{Specification of OnBoardConnectRep Component}
\end{crRep}
