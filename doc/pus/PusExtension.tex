...
%---------------------------------------------
% Define Environment for Command and Report
% Par #1: The string identifying the category to which the cmd/rep belongs
% Par #2: The table caption
%---------------------------------------------
\newenvironment{cr_cr}[2]
{
\begin{longtable}{|p{3cm}|p{10cm}|}
\caption{#2}\label{tab:CR-#1} \\
\hline
\rowcolor{light-gray}
\textbf{Item} & \textbf{Specified Value}\\
\hline\hline
\endfirsthead
\rowcolor{light-gray}
\textbf{Item} & \textbf{Specified Value}\\
\hline\hline
\endhead
\DTLforeach*[\DTLiseq{\cat}{#1}]{dbCmdRep}{\cat=Category,\ap=AP,\defValue=DefValue}
{\DTLiffirstrow{}{\\\hline}\ap\ & \defValue}\\\hline
}
{\end{longtable}}
...
\DTLloaddb{dbCmdRep}{PusExtensionCmdRep.csv}
...
The components of the CORDET Framework are defined as models which comply with the FW Profile of [FW-SP]. Figure \ref{fig:CrFwInCommand} shows the model of the InCommand (the figure is taken from [CR-SP]). This consists of a state machine where some guards and actions are marked as "Adaptation Points". Concrete commands are defined by attaching a concrete behaviour to these actions and guards.

Thus, for each supported PUS command, the framework extension defines an extension of the InCommand component which closes all the InCommand adaptation points and, for each supported PUS report, it defines an extension of the OutComponent component which closes all the OutComponent adaptation points.
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.415,keepaspectratio=true]{CrFwInCommand.png}
...
\item The adaptation point ICM-3 implements the acceptance check for the command. This consists of a generic part which verifies the correctness of the command length and CRC and a command-specific part which is implemented by a Validity Check which is defined for each command (typically, the validity check verifies the legality of the values of the command parameters).
...
HkDelete & Delete Housekeeping Parameter Report Structure & (3,3) \\
\hline
HkEnable & Enable Periodic Generation of Housekeeping Parameter Rep. & (3,5) \\
\hline
HkDisable & Disable Periodic Generation of Housekeeping  Parameter Rep. & (3,6) \\
\hline
HkEnable & Enable Periodic Generation of Diagnostic Parameter Rep. & (3,7) \\
\hline
HkDisable & Disable Periodic Generation of Diagnostic Parameter Rep. & (3,8) \\
\hline
...
\begin{cr_cr}{S1-A}{Specification of Service 1 Reports (Request Verification)}
\end{cr_cr}
