...
\item The Update Action of the incoming report AreYouAliveRep sets \texttt{AreYouAliveSrc} to be equal to its source
...
Note that one implication of this mechanism is that only one Is-Application-Process-Alive Test may be active at a given time (i.e. the user should only send a new IsAppProcAliveCmd command to an application after execution of the previous IsAppProcAliveCmd command has completed). This constraint is not enforced by the application and is under the responsibility of the user of the service.
