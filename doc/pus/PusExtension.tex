...
The Time-Based Scheduling Service controls the Time-Based Scheduling Function. This function allows time-tagged requests to be pre-loaded in an application and to be released when their time of execution comes due. The pre-loaded requests are held in the \textit{Time-Based Schedule} or TBS. The consists of a list of up to SCD\_N\_TBA Time-Based Activities or TBAs. Within the TBS, each TBA is identified by an integer in the range 1 to SCD\_N\_TBA. Each TBA is defined by the attributes listed in table \ref{tab:tbaAtt}. 



TBD: can the release time be earlier than the current time?

The entries in the TBS are arranged in a random order which is determined by the order in which the TBAs are loaded in the application. Each TBA has a pointer to the "next TBA" (attribute \texttt{nextTba}) which is defined as the TBA which is due for release next. This attribute allows the TBAs to be navigated in the order in which they are due for release. Additionally, and also in order to facilitate the scanning of the TBAs according to the their order of release time, variable \texttt{firstTba} is maintained. This variable points to the TBA with the next release time (i.e. to the first TBA which is due for execution).  

Within the TBS, up to SCD\_N\_SUB\_TBS sub-schedules may be defined. Each sub-schedule is identified by an integer in the range 1 to SCD\_N\_SUB\_TBS. The sub-schedule 1 is the default sub-schedule. Thus, an application which does not need sub-schedule should set SCD\_N\_SUB\_TBS to 1. To each TBA, the attribute \texttt{subSchedId} is associated which identifies the sub-schedule to which the TBA belongs.
...
\texttt{relTime} & The release time of the time-based activity & A valid on-board time value \\
\hline
\texttt{nextTba} & The identifier within the TBS of the time-based activity with the next release time & Integer in range 1..SCD\_N\_TBA \\
\hline
