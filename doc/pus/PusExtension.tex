...
%---------------------------------------------
% Define Environment for Command Tables 
...
\begin{cr_rep}{S3-25s}{Specification of HkOneShotRep Component}
\end{cr_rep}
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.55,keepaspectratio=true]{CrPsCmd3s27Prgr.png}
 \caption{Progress Action of Command HkOneShotCmd}
 \label{fig:Cmd3s27Prgr}
\end{figure}
...
The parameter monitoring subservice controls the \textit{parameter monitoring function}. Generally speaking, this function is used to monitor the values of a set of data items in the data pool. Each monitored value is checked periodically to verify whether it conforms to a certain pattern of behaviour (e.g. whether it remains within certain limits). Violations of this pattern of behaviour are reported through service 5 events. The behaviour of the parameter monitoring function is described by the Parameter Monitoring Procedure of figure \ref{fig:parMonPr}. This procedure should be executed cyclically with a period of MON\_PER by the host application.

The parameter monitoring function can be either enabled or disabled. When the function is enabled, it periodically processes a \textit{parameter monitoring definition list}. This consists of a list of \textit{parameter monitors}. Each parameter monitor defines a monitoring action for a data pool item. The same data pool item may be the object of several parameter monitors.

An application maintains a \textit{parameter monitoring list} which holds all the parameter monitors currently active in the application. The size of the list (i.e. the maximum number of parameter monitors which may be defined in an application) is statically fixed and is equal to MON\_N\_PMON. Service 12 offers the means to report the content of the parameter monitoring list and to alter its content.

A parameter monitor is characterized by the attributes listed in table \ref{tab:pmonAtt}. The first two items in the list are the identifiers of, respectively, the parameter monitor itself and the data item whose value the parameter monitor checks.

The monitoring action is performed by a \textit{parameter monitoring procedure}. Thus, for instance, there may be a parameter monitoring procedure which verifies that a parameter has a pre-defined value or there may another procedure which verifies that a parameter remains within pre-defined limits. Parameter monitoring procedures are described in the next sub-section.

Parameter monitors can be individually enabled and disabled. When a parameter monitor is disabled, no monitoring is performed on that parameter. Note that this means that the enable status of a monitoring parameter can be controlled at global level (the entire moniotring function is enabled/disabled) or at local level (a specific parameter monitor is enabled/disabled).

The parameter monitoring function is executed cyclically. Individual parameter monitors may either be executed every time the monitoring function is executed or they may be executed only every N executions of the monitoring functions. The value of N (a positive integer) is the period of the parameter monitor.

When a parameter monitor is executed, its monitoring procedure is run on the current value of the parameter. Based on the procedure outcome and on the repetition count for the parameter, the monitoring parameter status and sub-status are updated. The status can have one of three values:

\begin{itemize}
\item MON\_UNCHECKED: the monitoring procedure has not yet been run since the parameter monitoring function or the parameter monitor was last enabled 
\item MON\_VALID: the latest execution of the monitoring procedure found the parameter value to be nominal
\item MON\_SUSPECTED: the latest execution of the monitoring procedure found the parameter value to be non-nominal but the number of consecutive non-nominal outcomes is smaller than the repetition count
\item MON\_INVALID: the latest execution of the monitoring procedure found the parameter value to be non-nominal and the number of consecutive non-nominal outcomes is equal to or larger than the repetition count
\end{itemize}

The sub-status provides information about the outcome of the latest execution of the monitoring procedure. Its range of values depends on the monitoring procedures and is discussed in the next sub-section.

The monitoring procedure may need parameters. These are stord 
This is explained in more detail in the next sub-section but the basic mechanism is that a parameter is declared to be invalid if its monitoring procedure has found its value to be invalid in more than N consecutive execution where N is the repetition count.


\begin{pnptable}{|l|p{6.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|}{Attributes of Parameter Monitor}{tab:pmonAtt}{\textbf{Name} & \textbf{Description} & \textbf{Constraint}}
\texttt{pmonId} & Parameter monitor identifier & Integer in range: 1..MON\_N\_PMON \\
\hline
\texttt{dataItemId} & Identifier of the data item monitored by the parameter monitor & Integer in range: 1..HK\_MAX\_ID \\
\hline
\texttt{monPrId} & Identifier of the parameter monitoring procedure which is applied by the parameter monitor & A valid monitoring procedure identifier \\
\hline
\texttt{isEnabled} & Enable flag for the parameter monitor & Boolean value \\
\hline
\texttt{status} & Status of the parameter monitor & One of UNCHECKED, VALID, SUSPECTED or INVALID \\
\hline
\texttt{subStatus} & Sub-status of the parameter monitor & See table \ref{tab:retValMonPr} \\
\hline
...
\hline
\texttt{perCnt} & The phase counter & Positive integer \\
\hline
\texttt{repNmb} & The repetition number for the monitoring check  & Integer in range: 0..\texttt{per} \\
\hline
\texttt{repCnt} & The repetition counter for the monitoring check  & Integer in range: 0..\texttt{repNmb} \\
\hline
\texttt{eid} & The identifier of the event report generated if the parameter status becomes invalid & Integer in range: 1..EVT\_N\_EID \\
\hline
\end{pnptable}  


%=============================================================================================
\subsection{Parameter Monitoring Procedures}\label{sec:parMonProc}
The parameter monitoring procedures are responsible for checking the values of the monitored parameters and determining whether or not they are valid. The definition of the monitoring procedures is an adaptation point for which the PUS Extension offers the following three options:

\begin{itemize}
\item Limit Check Monitoring Procedure: verifies whether the value of the monitored parameter is within a pre-defined interval. See figure \ref{fig:MonLimCheckPr}.
\item Expected Value Monitoring Procedure: verifies whether the monitored parameter has  pre-defined value. See figure \ref{fig:MonExpValCheckPr}.
\item Delta Check Monitoring Procedure: verifies whether the difference between the current and previous value of the monitored parameter is within a pre-defined interval. See figure \ref{fig:MonDelCheckPr}.
\end{itemize}

Applications can add their own parameter monitoring procedures. It is an implementation-level decision whether the above procedure must be split into several instances, one for each syntactical type of the parameter to be checked (e.g. one instance for real-values parameters and another instane for integer-values parameters). 

To each parameter monitor, one instance of a parameter monitoring procedure is associated. The procedure is started when the parameter monitor is enabled (either because the entire parameter monitoring function is enabled or because the monitor itself is enabled) and then it is executed every time the parameter monitor is executed. At each execution, the procedure returns an outcome. If the procedure has found the parameter value to be nominal, it returns: MON\_VALID. If, instead, it has found the parameter value to be non-nominal, it returns some other value. The range of return values other than MON\_VALID is specific to each monitoring procedure. Table \ref{tab:retValMonPr} lists the potential outcomes of executing one of the three pre-defined monitoring procedures. Applications may  have to extend this range, depending on which monitoring procedure they define. 

\begin{pnptable}{|l|p{8cm}|}{Return Values of Monitoring Procedure Execution}{tab:retValMonPr}{\textbf{Name} & \textbf{Description}}
...
%----------------------------------------------------------------------------------------
\subsection{Service 12 Observables}\label{sec:serv5Obs}
...
\begin{cr_ap}{S12}{Adaptation Points for 12 (On-Board Monitoring Service)}
\end{cr_ap}
...
\begin{fw_itemize}
\item At application initialization time, the value of \texttt{largeMsgTransId} for the i-th LPT Buffer is initialized to: (i-1).
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.415,keepaspectratio=true]{CrPsLPT.png}
...
\appendix
%=============================================================================================
\section{Event Reports}\label{sec:preDefEvtRep}
The table in this section lists all the service 5 event reports which are generated by the PUS Extension of the CORDET Framework. For each event report, the following information is provided:
